...
Require Import Fiat.Bedrock.Nomega.
...
Definition weq : forall sz (x y : word sz), {x = y} + {x <> y}.
  ...
  abstract (rewrite (shatter_word y); simpl; intro; injection H; intros;
            try (apply _H0); apply inj_pair2_eq_dec in H0; [ auto | apply eq_nat_dec ]).
  
  abstract (rewrite (shatter_word y); simpl; intro; try apply _H; injection H; auto).
Defined.
